...
export async function registerRoutes(app: Express): Promise<Server> {
  app.post("/api/automate", async (req, res) => {
    try {
      ...
      const authMode = validatedData.authMode || process.env.AUTH_MODE || "standard";
      addLog("Initializing browser session", "running", undefined, `Using model: ${validatedData.model}, Auth: ${authMode}`);

      // Initialize Stagehand - use OAuth if authMode is "oauth"
      let stagehand: Stagehand;
      
      if (authMode === "oauth") {
        // OAuth + Custom Gateway mode
        try {
          const llmClient = await createStagehandLLMClient(validatedData.model);
          ...
          addLog("Using OAuth authentication", "success", undefined, "Custom gateway configured");
        } catch (oauthError) {
          const errorMsg = oauthError instanceof Error ? oauthError.message : "Unknown OAuth error";
          addLog("OAuth initialization failed", "error", undefined, errorMsg);
          throw new Error(`OAuth setup failed: ${errorMsg}. Check your .env configuration.`);
        }
      } else {
...
await stagehand.close();`;
      ...
      if (authMode === "oauth") {
        ...
        try {
          ...
          broadcastToSession(sessionId, {
            type: "error",
            sessionId,
            error: `OAuth setup failed: ${errorMsg}. Check your .env configuration.`,
          });
          return;
        }
      } else {
  ...
  app.get("/api/test-oauth", async (req, res) => {
    try {
      const { sendChatRequest } = await import("./lib/ai-client");
      
      const testMessages = [
        { role: "user" as const, content: "Hello, this is a test message. Please respond with 'OAuth connection successful!'" }
      ];
      
      const response = await sendChatRequest(testMessages, "gpt-4");
      ...
      res.status(500).json({
        ...
        hint: "Make sure all OAuth environment variables are set correctly in Replit Secrets",
      });
    }
  });
