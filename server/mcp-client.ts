...
export async function initMcpClient(): Promise<Client> {
  ...
  try {
    ...
    const configPath = process.env.XDG_CONFIG_HOME || path.join(tempDir, ".config");
    const browsersPath = process.env.PLAYWRIGHT_BROWSERS_PATH || path.join(tempDir, "playwright-browsers");
