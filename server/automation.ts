...
// Stagehand extends Playwright's Page with AI methods
type StagehandPage = Page & {
  act(action: string): Promise<any>;
  extract<T>(args: { instruction: string; schema: T; selector?: string }): Promise<T>;
...
export class AutomationExecutor {
  ...
  async initialize(model: AutomationModel): Promise<void> {
    ...
    try {
      ...
      this.stagehand = new Stagehand({
        env: "LOCAL",
        verbose: 1,
        model: model === "openai" ? "openai/gpt-4o" : model === "anthropic" ? "anthropic/claude-3-5-sonnet-20241022" : "google/gemini-2.0-flash-exp",
      });

      await this.stagehand.init();
      this.page = this.stagehand.context.pages()[0] as unknown as StagehandPage;
  ...
  async execute(prompt: string): Promise<AutomationResult> {
    ...
    try {
      ...
      // Use Stagehand's act method for general automation (v3 API takes string directly)
      this.log("Processing automation with AI", "running");
      const result = await this.page.act(prompt);
  ...
  async extract(prompt: string, instruction: string, schema?: any): Promise<AutomationResult> {
    ...
    try {
      ...
      if (prompt) {
        await this.page.act(prompt);
        this.log("Navigation completed", "success");
      }

      // Extract data (v3 API: takes single object with instruction and schema)
  ...
  async observe(instruction: string): Promise<AutomationResult> {
    ...
    try {
      ...
      // v3 API: observe takes instruction string directly
      const result = await this.page.observe(instruction);
