...
// Using gpt-4o-mini as recommended in Eko documentation
...
export async function executeAutomation(
  ...
  try {
    ...
    console.log('[eko-agent] Initializing Eko with OpenAI provider...');
    const eko = await initializeEko();
    console.log('[eko-agent] Eko initialized successfully');
    ...
    console.log('[eko-agent] Running task:', prompt);
    const result = await eko.run(prompt);
    console.log('[eko-agent] Task result:', JSON.stringify(result, null, 2));
    ...
    console.error('[eko-agent] Error:', error);
    console.error('[eko-agent] Error stack:', error.stack);
