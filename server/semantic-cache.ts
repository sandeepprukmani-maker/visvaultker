...
import { db } from "./db";
import { automationHistory, type InsertAutomationHistory, type AutomationHistory, type AutomationHistoryInsert } from "@shared/schema";
...
export async function saveAutomation(data: InsertAutomationHistory): Promise<void> {
  ...
  await db.insert(automationHistory).values({
    ...data,
    promptEmbedding: embedding ?? null,
  } as AutomationHistoryInsert);
}
