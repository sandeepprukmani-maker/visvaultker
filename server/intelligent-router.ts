...
const stagehand = new Stagehand({
  env: "LOCAL",
  model: "${model}", // v3 API: provider/model format
  verbose: 1,
});
...
// Execute action using v3 API
await stagehand.act("${task}");
...
// Discover elements using v3 API
const elements = await stagehand.observe("${task}");
...
// Extract data using v3 API  
const data = await stagehand.extract("${task}", z.object({
...
// Execute multi-step workflow using Stagehand v3 API
const agent = stagehand.agent({
  model: "${model}",
  systemPrompt: "You are a helpful automation assistant."
});

// Execute autonomous workflow (v3 API)
