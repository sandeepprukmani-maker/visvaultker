...
Scenario: Fill in the email field with test@example.com
...
def run_fill-in-the-email-field-with-testexamplecom(page: Page) -> bool:
    """
    ...
    """
    try:
            # Step 1: fill - Input field
            # Fill: Input field (with smart validation)
            if not smart_fill(page, 'page.get_by_label("Input field", exact=False)', "value from instruction"):
                print("⚠️  Warning: Failed to fill 'Input field'")
            # Expected: Field is filled

            # Step 2: execute - Perform automation task
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            # Run the automation
            success = run_fill-in-the-email-field-with-testexamplecom(page)
