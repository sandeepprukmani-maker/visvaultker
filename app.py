...
def fetch_epic(epic_id):
    ...
    try:
        ...
        if not user.jira_url or not user.jira_username or not user.jira_password:
            return jsonify({
                'error': 'Jira credentials not configured. Please configure Jira settings first.',
                'needs_config': True
            }), 400
        
        # Decrypt password
        password = decrypt_password(user.jira_password)
        if not password:
            return jsonify({'error': 'Failed to decrypt Jira password'}), 500
        
        # Use real Jira service
        jira = JiraService(user.jira_url, user.jira_username, password)
