...
Return ONLY valid JSON, no markdown formatting or explanations."""
    ...
    async def _generate_plan_with_llm(self, prompt: str, instruction: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(self.engine, 'llm') or hasattr(self.engine, 'chat_model') or hasattr(self.engine, 'client'):
            logger.info("   Using engine's LLM for intelligent planning")
            
            # Get LLM instance (check multiple possible attributes)
            llm = getattr(self.engine, 'llm', None) or getattr(self.engine, 'chat_model', None) or getattr(self.engine, 'client', None)
            
            if llm:
                try:
                    # Check if it's an OpenAI client (has chat.completions.create)
                    if hasattr(llm, 'chat') and hasattr(llm.chat, 'completions'):
                        logger.info("   Detected OpenAI client interface")
                        # Use OpenAI API directly
                        response = llm.chat.completions.create(
                            model=getattr(self.engine, 'model', 'gpt-4'),
                            messages=[
                                {"role": "system", "content": "You are an expert test automation planner. Generate plans in valid JSON format."},
                        ...
                        if content:
                            plan_json = self._extract_json_from_response(content)
                            if isinstance(plan_json, dict) and 'steps' in plan_json:
                                return plan_json
                    
                    # LangChain-style LLM (fallback)
                    elif hasattr(llm, 'apredict') or hasattr(llm, 'ainvoke'):
                        logger.info("   Detected LangChain-style LLM interface")
