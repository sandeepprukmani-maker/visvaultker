...
class WebsiteKnowledgeBase(db.Model):
    """Model for storing learned knowledge about websites to improve future automations"""
    ...
    common_elements = db.Column(db.Text, nullable=True)  # JSON: {login_button: {...}, search_box: {...}}
    navigation_patterns = db.Column(db.Text, nullable=True)  # JSON: {how_to_search: [...steps], how_to_login: [...steps]}
    form_patterns = db.Column(db.Text, nullable=True)  # JSON: {login_form: {...}, search_form: {...}}
    authentication_method = db.Column(db.String(100), nullable=True)  # oauth, username_password, etc.
    
    # Success metrics
    total_visits = db.Column(db.Integer, nullable=False, default=1)
    successful_automations = db.Column(db.Integer, nullable=False, default=0)
    failed_automations = db.Column(db.Integer, nullable=False, default=0)
    
    # Additional learned info
    notes = db.Column(db.Text, nullable=True)  # Free-form learned knowledge
    quirks = db.Column(db.Text, nullable=True)  # Known issues or special handling needed
    ...
    def to_dict(self):
        ...
        return {
            ...
            'common_elements': json.loads(self.common_elements) if self.common_elements else {},
            'navigation_patterns': json.loads(self.navigation_patterns) if self.navigation_patterns else {},
            'form_patterns': json.loads(self.form_patterns) if self.form_patterns else {},
