...
class EngineOrchestrator:
    ...
    def execute_instruction(self, instruction: str, engine_type: str, headless: bool, progress_callback=None) -> Dict[str, Any]:
        ...
        # Override engine if recommendation is stronger for this task
        original_engine = engine_type
        if analysis['recommended_engine'] != engine_type:
            logger.info(f"🔄 Intelligent override: Switching from {engine_type} to {analysis['recommended_engine']}")
            logger.info(f"   Reason: Better suited for {analysis['task_type']} tasks")
