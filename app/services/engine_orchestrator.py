...
from app.utils.credential_manager import credential_manager
...
class EngineOrchestrator:
    ...
    def __init__(self):
        ...
        self.enable_secure_credentials = config.getboolean('credentials', 'enable_secure_credentials', fallback=True)
        self.auto_inject_credentials = config.getboolean('credentials', 'auto_inject_credentials', fallback=True)
    ...
    def execute_instruction(self, instruction: str, engine_type: str, headless: bool, progress_callback=None) -> Dict[str, Any]:
        ...
        if self.enable_secure_credentials:
            instruction, credential_context = credential_manager.prepare_secure_instruction(instruction)
            
            if credential_context:
                logger.info(f"🔐 Secure credentials enabled for {credential_context.get('service')}")
                logger.info(f"🔒 Original instruction redacted from AI model")
                
                # Send credential injection event to frontend
                if progress_callback:
                    progress_callback('credential_injection', {
                        'service': credential_context.get('service'),
                        'message': f"Securely injecting credentials for {credential_context.get('service')} (hidden from AI)"
                    })
