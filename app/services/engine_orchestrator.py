...
class EngineOrchestrator:
    ...
    def __init__(self):
        ...
        self.script_generator = AIScriptGenerator(model_provider="oauth_gateway")
        self.dynamic_healer = DynamicHealerAgent(model_provider="oauth_gateway")
    ...
    def execute_instruction(self, instruction: str, engine_type: str, headless: bool, progress_callback=None, agent_mode: str = "direct", enable_agents: bool = True) -> Dict[str, Any]:
        """
        Execute an instruction using the specified engine with Playwright Test Agents workflow
        
        Engine Behavior:
        - 'playwright_mcp': Follows official Playwright Test Agents workflow
          * 🎭 Planner: Creates test plan with validated steps
          * 🎭 Generator: Generates Playwright code with verified locators
          * ⚡ Executes the automation
          * 🩺 Healer: Auto-fixes failures (if execution fails)
          Returns: Execution results + generated reusable code + healed code (if needed)
        
        - 'browser_use': Fast execution mode
          * Immediately executes using AI-powered browser automation
          * Optionally generates code if enable_agents=True
          Returns: Execution results + optional generated code
        
        Args:
            instruction: Natural language instruction
            engine_type: 'playwright_mcp' (Playwright Test Agents) or 'browser_use' (fast mode)
            headless: Run in headless mode
            progress_callback: Optional callback function for progress updates
            agent_mode: Execution mode (default: "direct")
            enable_agents: Whether to use Planner/Generator/Healer agents (auto-enabled for playwright_mcp)
            
        Returns:
            Dict with execution results, generated code (playwright_code), and healed code (if applicable)
        """
        ...
        # Force agents to be enabled for Playwright MCP (follows Playwright Test Agents workflow)
        # This enables: Planner → Generator → Execute → Healer (if fails)
        if engine_type == 'playwright_mcp':
            enable_agents = True
            logger.info("🎭 Playwright MCP: Using Playwright Test Agents workflow (Plan → Generate → Execute → Heal)")
        
        # Disable agents for Browser-Use (user doesn't want code generation for Browser-Use)
        if engine_type == 'browser_use':
            enable_agents = False
            logger.info("🚀 Browser-Use: Direct execution mode (no code generation)")
        ...
        # Use enhanced workflow if enabled (only for Playwright MCP)
        ...
        try:
            if engine_type == 'playwright_mcp':
                # Playwright Test Agents workflow: Generate code, then execute
                ...
                try:
                    ...
                    # Execute using direct mode (agent will handle the automation)
                    logger.info("🎭 Executing automation with Playwright MCP...")
                    result = agent.execute_instruction(instruction, mode=agent_mode, progress_callback=progress_callback)
                except Exception as e:
            ...
            if result is not None:
                ...
                # If execution failed and code was generated, use Healer to fix it
                # This follows the Playwright Test Agents workflow: Plan → Generate → Execute → Heal
            ...
            # Try to heal the script even on exception if agents are enabled and code was generated
