...
class KnowledgeRetriever:
    ...
    def get_knowledge_for_crawl(self, crawl_id: int, min_confidence: float = 0.5) -> Dict[str, Any]:
        """
        Get all knowledge for a specific crawl by ID
        ...
        Returns:
            Dictionary with knowledge data and site structure
        """
        ...
        knowledge_entries = SiteKnowledge.query.filter(
            SiteKnowledge.crawl_id == crawl_id,
            SiteKnowledge.confidence_score >= min_confidence
        ).all()
        
        pages = CrawlPage.query.filter_by(crawl_id=crawl_id).all()
        ...
        result = {
            ...
            "knowledge_entries": [entry.to_dict() for entry in knowledge_entries],
            "site_structure": self._get_site_structure(crawl),
            "pages": [page.to_dict(include_elements=False) for page in pages]
        }
        
        logger.info(f"📚 Retrieved {len(knowledge_entries)} knowledge entries for crawl #{crawl_id}")
