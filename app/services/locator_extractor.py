...
class IntelligentLocatorExtractor:
    ...
    def _extract_role_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if name:
            ...
            if role == 'button':
                return (LocatorStrategy.ROLE, f"page.get_by_role('button', name='{name}')")
            elif role == 'link':
                return (LocatorStrategy.ROLE, f"page.get_by_role('link', name='{name}')")
            elif role == 'textbox':
                return (LocatorStrategy.ROLE, f"page.get_by_role('textbox', name='{name}')")
            elif role == 'checkbox':
                return (LocatorStrategy.ROLE, f"page.get_by_role('checkbox', name='{name}')")
            elif role == 'heading':
                return (LocatorStrategy.ROLE, f"page.get_by_role('heading', name='{name}')")
            else:
                return (LocatorStrategy.ROLE, f"page.get_by_role('{role}', name='{name}')")
        else:
            # Role without name (less specific)
            return (LocatorStrategy.ROLE, f"page.get_by_role('{role}')")
    ...
    def _extract_label_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if label:
            ...
            return (LocatorStrategy.LABEL, f"page.get_by_label('{label}')")
    ...
    def _extract_placeholder_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if placeholder:
            ...
            return (LocatorStrategy.PLACEHOLDER, f"page.get_by_placeholder('{placeholder}')")
    ...
    def _extract_testid_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if test_id:
            return (LocatorStrategy.TEST_ID, f"page.get_by_test_id('{test_id}')")
    ...
    def _extract_text_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if text and len(text) > 0 and len(text) < 100:  # Reasonable text length
            ...
            if len(text) < 50:
                return (LocatorStrategy.TEXT, f"page.get_by_text('{text}')")
            else:
                # Use partial match for long text
                partial = text[:30]
                return (LocatorStrategy.TEXT, f"page.get_by_text('{partial}', exact=False)")
    ...
    def _extract_alt_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if alt:
            ...
            return (LocatorStrategy.ALT_TEXT, f"page.get_by_alt_text('{alt}')")
    ...
    def _extract_title_locator(self, element_info: Dict) -> Optional[Tuple[LocatorStrategy, str]]:
        ...
        if title:
            ...
            return (LocatorStrategy.TITLE, f"page.get_by_title('{title}')")
