...
function getAutomationSettings() {
    return {
        mode: localStorage.getItem('automation_mode') || 'headful',
        engine: localStorage.getItem('automation_engine') || 'browser_use'
    };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize visibility on page load based on saved engine setting
    const settings = getAutomationSettings();
    toggleScriptPanels(settings.engine);
    ...
    if (currentTheme === 'light') {
        ...
        if (themeToggleBtn) {
            themeToggleBtn.querySelector('i').setAttribute('data-feather', 'sun');
            feather.replace();
        }
    }
        ...
        executeBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const payload = {
                    ...
                    headless: settings.mode === 'headless'
                };
                ...
                while (true) {
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            if (data.type === 'start') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">🚀 ${data.message || 'Starting...'}</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                                
                                // Clear screenshot panel for new execution
                                document.getElementById('screenshot-panel').innerHTML = '<div class="empty-state"><div>Screenshots will appear here during execution</div></div>';
                            } else if (data.type === 'init') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">⚙️  Initializing automation...</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'browser_init') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">🌐 Browser ready</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'agent_create') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">🤖 AI agent ready</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'execution_start') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">▶️  Executing steps...</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'planner_init') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">🎭 Planning automation...</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'generator_init') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">🎨 Preparing code generation...</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'generator_building') {
                                stepLogs.push(`<div style="color: var(--text-secondary); margin: 3px 0;">🔧 Generating Python script...</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'generator_complete') {
                                stepLogs.push(`<div style="color: var(--success-text); margin: 3px 0;">✓ Script generated (${data.data.lines} lines)</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'healer_init') {
                                stepLogs.push(`<div style="color: var(--warning-text); margin: 3px 0;">🔧 Healing script...</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'healer_complete') {
                                stepLogs.push(`<div style="color: var(--success-text); margin: 3px 0;">✓ Script healed (${data.data.fixes} fixes applied)</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                            } else if (data.type === 'step') {
                                stepLogs.push(`<div style="color: var(--success-text); margin: 3px 0;">✓ ${data.data.action}</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                                ...
                                stepLogs.push(`<div style="color: var(--success-text); margin: 3px 0;">📸 Screenshot saved</div>`);
                                document.getElementById('execution-logs').innerHTML = stepLogs.join('');
                                ...
                                // Add final summary
                                stepLogs.push(`<div style="color: var(--success-text); margin-top: 10px; font-weight: bold;">✅ Completed (${result.iterations || 0} steps)</div>`);
                                ...
                                if (isVerificationError) {
                                    ...
                                    stepLogs.push(`<div style="color: var(--error-text); font-weight: bold; margin-top: 10px; padding: 10px; border: 2px solid var(--error-text); border-radius: 4px; background: rgba(220, 38, 38, 0.1);">⚠️ VERIFICATION FAILED</div>`);
