...
class MCPExecutorAgent:
    ...
    async def execute_task(self, task_description: str, headless: bool = True) -> Dict[str, Any]:
        """Execute a browser automation task with enhanced tracing"""
...
- What you observed"""
    ...
    def _extract_locator_info(self, tool_name: str, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        """Extract locator information from tool arguments for better code generation"""
        locator_info = {
            "action": tool_name,
            "selectors": []
        }
        
        # Extract selector information from common playwright actions
        if "selector" in tool_args:
            selector = tool_args["selector"]
            locator_info["selectors"].append({
                "type": "css" if not selector.startswith("//") else "xpath",
                "value": selector
            })
        
        # Extract text-based locators
        if "text" in tool_args:
            locator_info["selectors"].append({
                "type": "text",
                "value": tool_args["text"]
            })
        
        # Extract role-based locators
...
class HealerAgent:
    """Agent that fixes failed Playwright scripts using trace analysis and multi-locator strategies"""
    ...
    async def heal_code(
        ...
        """
        logger.info("🔧 Healer Agent starting code repair...")
