...
class HealerAgent:
    ...
    def _analyze_with_llm(
        ...
        prompt = f"""Analyze this Playwright Python script error and suggest a fix:
...
{f"**Trace File**: {trace_path}" if trace_path else ""}

Analyze the error and provide:
1. **error_type**: Category of error (timeout, locator, network, etc.)
2. **fixable**: Boolean - can this be automatically fixed?
3. **fix_description**: Brief description of the fix
4. **fix_strategy**: How to fix (increase_timeout, fix_locator, add_wait, etc.)
5. **reason**: If not fixable, explain why
...
```"""
    ...
    def _apply_fix_with_llm(self, code: str, analysis: Dict[str, Any]) -> str:
        ...
        prompt = f"""Fix this Playwright Python script based on the error analysis:
...
Apply the necessary fixes and return the complete, fixed Python script.
