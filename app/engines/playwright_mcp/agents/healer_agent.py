...
{{"fixable": bool, "error_type": "timeout|locator|network|state", "fix_description": "...", "fix_strategy": "increase_timeout|fix_locator|add_wait|strict_mode", "reason": "..."}}"""
    ...
    def _fix_locator(self, code: str, analysis: Dict[str, Any]) -> str:
        """Fix broken locators by trying alternative strategies"""
        import re
        
        # Common locator fixes:
        # 1. Add .first() to handle multiple matches
        # 2. Increase timeout
        # 3. Add wait_for visibility
        # 4. Try alternative locator strategies
        
        # Extract the failing locator from analysis if provided
        error_type = analysis.get('error_type', '')
        
        if 'strict mode' in error_type.lower() or 'multiple' in error_type.lower():
            # Add .first() to resolve strict mode violations
            return self._fix_strict_mode(code)
        
        # Try adding explicit waits before locators
        # Find patterns like: await page.get_by_* with proper indentation capture
        # Capture leading whitespace to preserve indentation
        pattern = r'^(\s*)(await\s+page\.(get_by_\w+|locator)\([^)]+\))'
        
        def add_wait(match):
            indent = match.group(1)  # Capture original indentation
            locator_call = match.group(2)  # The actual locator call
            # Don't modify if already has wait_for
            if 'wait_for' in locator_call:
                return match.group(0)  # Return unchanged
            # Add wait for visible state, preserving indentation
            return f'{indent}{locator_call}.wait_for(state="visible", timeout=10000)\n{indent}{locator_call}'
        
        # Apply the fix (MULTILINE to match ^ at line start)
        fixed_code = re.sub(pattern, add_wait, code, count=1, flags=re.MULTILINE)
        
        return fixed_code if fixed_code != code else code
