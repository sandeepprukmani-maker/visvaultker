            ...
            while iteration < max_iterations:
                ...
                if not has_tool_calls:
                    ...
                    if plan_data:
                        ...
                        logger.info("✅ Planner Agent: Successfully created automation plan with validated locators")
                        return {
            ...
            if isinstance(element_data, dict):
                ...
                try:
                    ...
                    if is_valid:
                        ...
                        logger.info(f"✅ Validated locator for step {step.get('step_number')}: {reason}")
                    else:
                        ...
                        element_data['validation_reason'] = reason
                        logger.warning(f"⚠️  Locator needs improvement for step {step.get('step_number')}: {reason}")
                except Exception as e:
                    logger.warning(f"⚠️  Error generating locator for step {step.get('step_number')}: {e}")
