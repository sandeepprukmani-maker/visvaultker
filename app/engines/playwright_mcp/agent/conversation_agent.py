...
class BrowserAgent:
    ...
    def __init__(self, mcp_client: Any, workspace_root: str = "."):
        ...
        # True: Integrated workflow (Plan → Generate → Execute → Heal)
        self.generate_python_code = True
        
        # Execution strategy
        # "direct": BrowserAgent directly controls browser (no code gen)
        # "code_first": Plan → Generate code → Execute code → Heal if needed
...
═══════════════════════════════════════════════════════════════════════════════"""
    ...
    def _execute_code_first_mode(self, instruction: str, progress_callback=None) -> Dict[str, Any]:
        """
        Execute using integrated three-agent workflow: Plan → Generate → Execute → Heal
        ...
        try:
            ...
            if progress_callback:
                progress_callback('planning', {'message': '🎭 Planner: Creating automation plan...'})
            
            self.logger.info("🎭 Step 1/3: Planning automation strategy...")
            ...
            if progress_callback:
                progress_callback('generating', {'message': '🎨 Generator: Writing Python code...'})
            
            self.logger.info("🎨 Step 2/3: Generating Python Playwright script...")
            ...
            if progress_callback:
                progress_callback('executing', {'message': '⚡ Executing automation task...'})
            
            self.logger.info("⚡ Step 3/3: Executing automation...")
            ...
            self.logger.error(f"❌ Integrated workflow failed: {e}", exc_info=True)
            return {
                "success": False,
                "error": str(e),
