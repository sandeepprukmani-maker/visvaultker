...
class BrowserAgent:
    ...
    def __init__(self, mcp_client: Any, workspace_root: str = "."):
        ...
        self.temperature = float(config.get('openai', 'temperature', fallback='0.7'))
        ...
        self.healer_agent = HealerAgent(llm_client=self.client, temperature=self.temperature)
        
        # Enable Python code generation (planner/generator/healer chain)
        # PERFORMANCE: This adds significant overhead - disable for faster direct execution
        self.generate_python_code = config.getboolean('playwright_mcp', 'generate_python_code', fallback=False)
        
        # Performance optimizations
        self.enable_resource_blocking = config.getboolean('playwright_mcp', 'enable_resource_blocking', fallback=True)
        self.include_snapshots = config.getboolean('playwright_mcp', 'include_snapshots', fallback=False)
        
        # Apply resource blocking if enabled (25-50% faster page loads)
        if self.enable_resource_blocking:
            try:
                result = self.mcp_client.enable_resource_blocking()
                self.logger.info(f"⚡ Resource blocking enabled: {result.get('speedup', 'unknown')} speedup expected")
            except Exception as e:
                self.logger.warning(f"⚠️  Could not enable resource blocking: {e}")
    ...
    def _execute_direct_mode(self, instruction: str, progress_callback=None) -> Dict[str, Any]:
        ...
        self.conversation_history = [
            {
                "role": "system",
                "content": """You are a helpful browser automation assistant that understands natural language.

Your approach:
1. Understand what the user wants - interpret their request flexibly
2. Navigate, click, fill forms, and extract data as needed
3. When you see page snapshots (YAML format), use element references like [ref=e1] for interactions
4. Work efficiently - complete the task without unnecessary extra steps
5. Once done, respond with a completion message
6. Move forward with confidence - don't repeat successful actions
...
Verification tasks:
If the user asks you to verify, check, or confirm something:
- Actually check the condition using browser tools
- Report clearly: "Verification passed" or "Verification failed: [reason]"
- Example: "check if button exists" → take snapshot, look for button, report result

Be helpful, adaptive, and efficient."""
            },
            {
                "role": "user",
                "content": instruction
