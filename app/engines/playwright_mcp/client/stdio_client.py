...
class MCPStdioClient:
    ...
    def __init__(self, headless: bool = True, browser: str = 'chromium'):
        ...
        assert self.process.stdin is not None, "stdin not initialized"
        assert self.process.stdout is not None, "stdout not initialized"
        assert self.process.stderr is not None, "stderr not initialized"
        ...
        # Reduced from 200ms to 50ms for faster startup
        time.sleep(0.05)
    ...
    def _read_stderr(self):
        ...
        try:
            while self.process and self.process.poll() is None and self.process.stderr:
                try:
    ...
    def _make_request(self, method: str, params: Optional[Dict] = None, timeout: int = 30) -> Dict:
        ...
        if not self.process.stdin:
            raise Exception("MCP server stdin not available")
    ...
    def cleanup(self):
        """Cleanup the MCP client and subprocess (alias for close)"""
        self.close()
