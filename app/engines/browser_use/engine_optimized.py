...
from app.engines.browser_use.code_generator import BrowserUseCodeGenerator
...
class OptimizedBrowserUseEngine:
    ...
    def __init__(self, headless: bool = False, enable_advanced_features: bool = True):
        ...
        if enable_advanced_features:
            ...
            # Initialize code generator for creating reusable Python scripts
            self.code_generator = BrowserUseCodeGenerator(llm_client=self.llm)
            
            logger.info("🚀 Advanced features enabled: Screenshots, PDFs, Cookies, Retry, Performance Tracking, Code Generation")
        else:
...
- Confirm sensitive actions before executing
            ...
            if self.enable_advanced_features:
                ...
                # Generate Python Playwright code from execution history
                if result["success"] and len(steps) > 0:
                    try:
                        logger.info("🎨 Generating reusable Python Playwright code...")
                        if progress_callback:
                            progress_callback('code_generation', {
                                'message': 'Generating Python Playwright script...'
                            })
                        
                        code_result = self.code_generator.generate_script(
                            instruction=instruction,
                            steps=steps,
                            final_result=final_result
                        )
                        
                        result["generated_code"] = code_result.get('python_code', '')
                        result["code_metadata"] = code_result.get('metadata', {})
                        
                        logger.info(f"✅ Generated {code_result.get('metadata', {}).get('lines_of_code', 0)} lines of Python code")
                        
                        if progress_callback:
                            progress_callback('code_generated', {
                                'message': 'Python script generated successfully!',
                                'lines': code_result.get('metadata', {}).get('lines_of_code', 0)
                            })
                    except Exception as code_gen_error:
                        logger.warning(f"⚠️  Code generation failed: {code_gen_error}")
                        result["generated_code"] = None
                        result["code_generation_error"] = str(code_gen_error)
