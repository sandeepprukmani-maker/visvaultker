...
- Confirm sensitive actions before executing
            ...
            # Capture screenshot if save_screenshot is True
            screenshot_paths = []
            if save_screenshot and self.enable_advanced_features and browser:
                try:
                    # Get the current page from browser (await async method)
                    page = await browser.get_current_page()
                    if page:
                        screenshot_result = await self.advanced_features.capture_screenshot(page, name="final")
