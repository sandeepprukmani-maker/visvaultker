...
class OptimizedBrowserUseEngine:
    ...
    def __init__(self, headless: bool = False, enable_advanced_features: bool = True):
        ...
        if use_chat_browser_use and CHAT_BROWSER_USE_AVAILABLE:
            ...
            logger.info(f"Using gateway model: {model} via {gateway_base_url} (temperature={temperature})")
            self.llm = ChatOpenAI(
                model=model,
    ...
    async def execute_instruction(self, instruction: str, 
        ...
        try:
            ...
            # Flexible system instructions for natural language understanding
            system_instructions = f"""
YOUR TASK: "{instruction}"

You're a helpful browser automation assistant. Understand what the user wants and complete it efficiently.

Key Principles:
1. Understand the user's goal - interpret their natural language instruction flexibly
2. Complete the task as requested - don't do extra actions they didn't ask for
3. Handle common issues automatically:
   - Wait for pages and elements to load
   - Handle popups that appear
   - Retry if elements take time to appear
   - Navigate through intermediate steps as needed

Examples to guide you:
- "go to linkedin" → Navigate to linkedin.com and confirm page loaded
- "search amazon for laptop" → Go to amazon, find search, type "laptop", submit search
- "fill email and submit" → Fill the email field AND click submit button

When to stop:
- Once you've completed what the user asked for, use done()
- Don't do extra helpful actions beyond the request
- If something fails after retries, report it clearly and use done()

Handling errors:
- If an element isn't found, the page might still be loading - wait and retry
- Verify important actions worked before moving on
- Be adaptive - try different approaches if needed
