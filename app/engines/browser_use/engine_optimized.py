...
- Confirm sensitive actions before executing
            ...
            # Automatic screenshot after last step (unless user explicitly mentions screenshot in prompt)
            screenshot_paths = []
            user_mentioned_screenshot = any(keyword in instruction.lower() for keyword in ['screenshot', 'capture', 'snap'])
            auto_screenshot = not user_mentioned_screenshot and self.enable_advanced_features
            
            if (save_screenshot or auto_screenshot) and browser:
                try:
                    # Get the current page from browser (await async method)
                    page = await browser.get_current_page()
                    if page:
                        screenshot_name = "final" if save_screenshot else "auto_final"
                    ...
                    logger.warning(f"⚠️  Screenshot capture failed: {screenshot_error}")
