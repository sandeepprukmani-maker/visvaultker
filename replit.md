...
The application supports multiple AI providers configurable via `config/config.ini`:

**Supported Providers:**
1. **OpenAI** (default) - Requires `OPENAI_API_KEY`
2. **Anthropic** - Requires `ANTHROPIC_API_KEY`
3. **Google Gemini** - Requires `GOOGLE_API_KEY`
4. **OAuth Gateway** - Requires OAuth credentials

To configure your provider, edit `config/config.ini`:
```ini
[model_provider]
provider = openai  # or anthropic, gemini, oauth_gateway
model = gpt-4o     # or any provider-specific model
```
...
## Recent Changes
- **2025-11-04**: Enhanced with flexible multi-provider AI support (COMPLETE)
  - Created `ModelProviderFactory` (`app/utils/model_provider.py`) for unified provider management
  - Created `OpenAIAdapter` (`app/utils/openai_adapter.py`) to wrap langchain models with OpenAI-compatible interface
  - Added support for four AI providers: OpenAI, Anthropic, Gemini, and OAuth Gateway
  - **Technical Implementation**:
    - browser_use engine: Uses langchain chat models directly (ChatOpenAI, ChatAnthropic, ChatGoogleGenerativeAI)
    - playwright_mcp engine: Uses OpenAI-adapted langchain models for compatibility with `.chat.completions.create()` interface
    - OpenAIAdapter converts OpenAI-style messages to langchain BaseMessage objects (SystemMessage, HumanMessage, AIMessage)
    - Provider-specific parameter handling (e.g., Gemini doesn't support timeout parameter)
  - Updated config.ini with new `[model_provider]` section for easy provider switching
  - Updated main.py to validate provider-specific credentials on startup
  - Installed langchain-anthropic and langchain-google-genai packages
  - **Usage**: Switch providers by simply editing `config/config.ini` - both engines work seamlessly with all providers
- **2025-11-04**: Completed project import and setup
...
### Known Limitations
1. **API Keys Required**: Users must provide appropriate API keys based on selected provider:
   - OpenAI: OPENAI_API_KEY
   - Anthropic: ANTHROPIC_API_KEY
   - Gemini: GOOGLE_API_KEY
   - OAuth Gateway: Full OAuth credentials
2. **Development Server**: Currently using Flask development server (use gunicorn for production)
3. **SQLite Database**: Suitable for development; consider PostgreSQL for production scale
4. **Provider-Specific Features**: Some advanced features (tool calling, function modes) may vary by provider
