...
This application supports Windows environments. Follow these steps to run it locally on Windows:
...
4. **Set environment variables:**
   ...
   The app uses `dotenv` to automatically load variables from `.env` on startup.
...
5. **Run the development server:**
   ...
   Or with cross-env (already installed):
   ```powershell
...
The following changes ensure Windows compatibility:

1. **MCP Client (`server/mcp-client.ts`):**
   - Uses `os.tmpdir()` instead of hardcoded `/tmp` paths
   - Uses `path.join()` for cross-platform path separators
   - Detects Windows and uses `npx.cmd` instead of `npx`

2. **Environment Variables:**
   - `dotenv` package loads variables from `.env` file automatically
   - `cross-env` package is installed for cross-platform env var support
   - Scripts can use `cross-env NODE_ENV=value` syntax
...
- **"npx is not recognized"**: Ensure Node.js bin directory is in your PATH
- **Playwright browser issues**: Run `npx playwright install chromium` with admin privileges if needed
- **Permission errors**: Run PowerShell as Administrator for first-time setup