...
## System Architecture
The application uses Python 3.11 with the Flask web framework, SQLite for local data storage, and Playwright with Chromium for browser automation. It integrates with a Node.js-based Playwright MCP server for enhanced automation capabilities.

### Replit Environment Setup
- **Python Version**: Python 3.11
- **Node.js Version**: Node.js 20
- **Development Server**: Flask development server on port 5000 (host: 0.0.0.0)
- **Production Server**: Gunicorn with 4 workers on port 5000
- **Deployment**: Configured for Replit Autoscale deployment
- **Workflow**: Configured to run `python main.py` with webview output on port 5000

### Key Features
- **Browser Automation**: AI-guided web browsing actions.
- **Multiple AI Engines**: Supports `browser-use` and `Playwright MCP`.
- **Site Crawler & Knowledge Base**: Intelligent website crawler that learns site structure, elements, and interactions using browser-use AI. The crawler:
  - **Deep DOM Analysis**: Extracts ALL interactive elements (buttons, links, inputs, forms) with accurate CSS selectors and XPath
  - **Element Hierarchy**: Captures parent-child relationships and element positioning for comprehensive structure understanding
  - **Smart Pagination Handling**: Detects and skips pagination links (Page 2, Page 3, Next, Previous) to avoid duplicate content while learning from page 1
  - **Credential Support**: Works with password-protected sites using stored credentials from the vault
  - **Structured Knowledge Base**: Compiles learned information into categories (site structure, form fields, interactive elements)
  - **Automation Enhancement**: Automatically provides context to AI agents for more accurate and reliable automation
...
- **Knowledge-Enhanced Automation**: Automatically uses crawled site knowledge to improve automation accuracy and reduce errors.
- **Web Interface**: User-friendly dashboard for managing automations and site crawls.
