...
Eko is an AI-powered web automation platform that transforms natural language prompts into executable multi-step workflows. Users describe automation tasks in plain English, and the system generates, visualizes, and executes workflows using Claude AI (Anthropic). The application provides real-time execution monitoring, workflow history, and a modern, gradient-accented UI built with React and shadcn/ui components.
...
- Synchronous workflow execution (no actual browser automation yet) - placeholder for future Playwright/Puppeteer integration
