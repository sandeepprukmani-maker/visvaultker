...
## Replit Environment Setup (November 10, 2025)

### Configured for Replit
- **Frontend Server**: Vite dev server configured to bind to 0.0.0.0:5000 with HMR over WSS
- **Backend Servers**: 
  - Node.js Express server on 0.0.0.0:5000 (serves both frontend and API)
  - Python FastAPI server on localhost:8000 (database operations)
- **Database**: SQLite (visionvault.db) with semantic caching using NumPy cosine similarity
- **Browser Automation**: Playwright/Stagehand configured to use Nix-installed Chromium browser
- **Dependencies**: 
  - Node.js packages managed via npm
  - Python packages managed via uv (virtual environment in .pythonlibs)
  - System dependencies (Chromium, X11 libs) installed via Nix
- **Workflow**: Single workflow running `start-servers.sh` which starts both Python and Node.js servers
- **Deployment**: Configured for VM deployment to maintain persistent state
...
The application uses SQLite for database storage via Python FastAPI backend (`server/main.py`). The database connection is managed by SQLAlchemy with models defined in `server/models.py`. Data models include:
- **AutomationHistory**: Stores execution history with prompt, mode, model, logs (JSON), generated code, screenshots (base64), and Gemini embeddings (768-dimensional vectors stored as JSON)
- Screenshot storage uses base64-encoded data URIs stored in text fields
- Semantic caching leverages cosine similarity (NumPy-based) on Gemini embeddings with 0.85 threshold for intelligent prompt matching
- SQLite database file: `visionvault.db` (automatically created on first run)
