...
**Core Features:**
- **Intelligent Automation Coordinator** ⭐ NEW: Every automation is analyzed before execution:
    - **Task Complexity Assessment**: Classifies tasks as Simple, Moderate, Complex, or Advanced
    - **Automatic Engine Selection**: Chooses optimal engine (Browser Use vs Playwright MCP) based on task type and complexity
    - **Intelligent Instruction Enhancement**: Adds strategic guidance, risk warnings, and step-by-step breakdowns
    - **Subtask Decomposition**: Automatically breaks complex instructions into sequential steps
    - **Risk Factor Identification**: Detects authentication, dynamic content, popups, and other challenges
    - **Fallback Strategy Generation**: Provides alternative approaches if primary method fails
    - **Task Type Detection**: Identifies login, search, data extraction, form filling, and other patterns
...
- **Enhanced History Feature with Auto-Healing**: Each execution stores screenshots, detailed logs, generated Python Playwright scripts, and automatically "healed" scripts. Users can re-execute scripts from history with a single click - the system automatically heals locator issues and saves improved versions. Healed scripts take precedence over generated scripts for maximum reliability.
...
- **Configuration**: Managed via `config/config.ini`, including:
  - **AI Temperature Control**: Configurable temperature (0.0-1.0) for all AI engines. Default 0.7 for flexible, conversational natural language understanding. Lower values (0.1-0.3) require more precise prompts. Higher values (0.6-0.9) enable flexible interpretation.
  - Browser performance settings, MCP server mode (persistent), max steps, and retry mechanisms.
