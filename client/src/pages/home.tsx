...
import type { Execution, Workflow, LogEntry, ExecutionStatus as ExecStatus, ModelOption } from "@shared/schema";
...
export default function Home() {
  ...
  const generateMutation = useMutation({
    mutationFn: async ({ prompt, model }: { prompt: string; model: ModelOption }) => {
      const response = await apiRequest("POST", "/api/workflow/generate", { prompt, model });
  ...
  const handleSubmitPrompt = (prompt: string, model: ModelOption) => {
    setLogs([]);
    setProgress(0);
    setWorkflow(null);
    addLog("info", `Starting automation: "${prompt}" using ${model === "anthropic" ? "Claude" : "GPT-4o-mini"}`);
