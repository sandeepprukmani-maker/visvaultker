...
export default function Home({ 
  ...
  useEffect(() => {
    if (activeTask) {
      setMessages((prevMessages) => {
        const existingUserMsg = prevMessages.find(
          (m) => m.role === "user" && m.taskId === activeTask.id
        );
        ...
        if (existingUserMsg) {
          ...
          const messagesBeforeTask = prevMessages.slice(0, userMsgIndex + 1);
          ...
          return assistantMessage 
            ? [...messagesBeforeTask, assistantMessage]
            : messagesBeforeTask;
        }
  ...
  const executeMutation = useMutation({
    ...
    onSuccess: (task, _prompt, context) => {
      setMessages((prev) => 
        prev.map((msg) => 
          msg.id === `user-${context?.tempId}` 
            ? { ...msg, id: `user-${task.id}`, taskId: task.id, timestamp: task.createdAt }
            : msg
        )
      );
      onNewTask(task);
