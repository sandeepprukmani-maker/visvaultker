...
export type AuthMode = "standard" | "oauth";
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const savedAuthMode = localStorage.getItem("stagehand-auth-mode") as AuthMode;
      if (savedAuthMode && ["standard", "oauth"].includes(savedAuthMode)) {
        setAuthModeState(savedAuthMode);
      }
  ...
  useEffect(() => {
    if (isInitialized && typeof window !== "undefined") {
      localStorage.setItem("stagehand-auth-mode", authMode);
    }
  ...
  return (
    <SettingsContext.Provider value={{ model, setModel, screenshotMode, setScreenshotMode, authMode, setAuthMode }}>
      {children}
